...
router.post('/upload', requireAuth, async (req, res) => {
  try {
    ...
    if (!folderId)  return res.status(400).json({ error: 'Cannot parse Drive folder ID' });
    ...
    const r = await uploadFile(req.session.googleTokens, job.result.filePath, folderId, job.result.fileName, '');
